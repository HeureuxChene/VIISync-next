...
const Index = ({ setPosition, screenWidth }) => {
    ...
    const [filtreTexte] = useState(['TOUT VOIR', 'IDENTITÉ VISUELLE', 'PACKAGING & ÉTIQUETTE', 'SITE INTERNET', 'DESIGN & MISE EN PAGE'])
    ...
    return (
        <>
            <section id='homeRef' className={style.homeRef}>
                <video id={style.homeRefVideo} autoPlay muted loop='infinite' src={`/media/video/ref-design/design-accueil.mp4`} />
                ...
                <section id={style.referencesSection}>
                    {
                        Object.keys(media).map((key, index) => {
                            return (
                                <Link key={index} href={`/design/${key}#topPage`} className={`${style.referencesLink} ${briqueFiltre[index] ? style.visible : style.invisible}`}>
                                    ...
                                    <Image className={style.referencesImg} alt={key} src={`/media/photo/ref-design/${key}/${media[key]['vignette']}`} width={1080} height={846} />
                                </Link>
                            )
                        })
                    }
